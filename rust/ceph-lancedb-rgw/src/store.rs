...
impl MultipartUpload for RGWMultipartUpload {
    fn put_part(
        ...
        // Pre-allocate slot for etag (will be filled by FFI)
        self.parts.push(String::new());
        let _part_index = self.parts.len() - 1;

        // This is a hack - we need to update the parts vector after the FFI call
        // but we can't do it from a 'static future. For now, we'll just ignore the etag.
        // A proper implementation would use Arc<Mutex<Vec<String>>> for parts.

...
extern "C" {
    ...
    ///
    /// # Arguments
    /// * `driver` - RGW driver pointer (env.driver in RGW handlers)
    /// * `dpp` - DoutPrefixProvider for logging
    /// * `bucket` - Bucket name (null-terminated)
    /// * `key` - Object key (null-terminated)
    /// * `data` - Pointer to data bytes
    /// * `len` - Length of data
    /// * `content_type` - MIME type (null-terminated)
    ///
    /// # Returns
    ...
    /// * `length` - Number of bytes to read (u64::MAX for entire object)
    /// * `buffer` - Output buffer (caller must free with rgw_free_buffer)
    ///
    /// # Returns
    /// 0 on success, -ENOENT if not found, other negative errno on failure
    ...
    ///
    /// # Returns
    /// 0 on success, -ENOENT if not found (treated as success), other negative errno on failure
    ...
    /// * `result` - Output result (caller must free with rgw_free_list_result)
...
//=============================================================================
// Re-export functions based on feature flag
//=============================================================================

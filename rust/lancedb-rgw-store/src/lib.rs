...
//! CephLanceDBRegistry* registry = rgw_lancedb_create_registry(driver, dpp);
//!
...
pub type RGWLanceDBRegistry = c_void;
...
/// - Free with rgw_lancedb_registry_free()
///
/// # Example (from C++)
/// ```cpp
/// // Create registry with RGW backend
/// void* registry = rgw_lancedb_create_registry(driver, dpp);
///
/// // Create session with custom cache sizes using lancedb-c API
/// LanceDBSessionOptions options = {
///     .index_cache_bytes = 512 * 1024 * 1024,    // 512 MB
...
/// ```
#[no_mangle]
pub unsafe extern "C" fn rgw_lancedb_create_registry(
    driver: *mut c_void,
    dpp: *const c_void,
) -> *mut RGWLanceDBRegistry {
    if driver.is_null() {
    ...
    Arc::into_raw(registry) as *mut RGWLanceDBRegistry
}

/// Free a registry created by rgw_lancedb_create_registry
///
...
///
/// # Safety
/// - `registry` must be a valid pointer returned by rgw_lancedb_create_registry
...
#[no_mangle]
pub unsafe extern "C" fn rgw_lancedb_registry_free(registry: *mut RGWLanceDBRegistry) {
    if !registry.is_null() {
...
#[no_mangle]
pub extern "C" fn rgw_lancedb_default_index_cache_size() -> usize {
    DEFAULT_INDEX_CACHE_SIZE
}

/// Get the default metadata cache size in bytes
#[no_mangle]
pub extern "C" fn rgw_lancedb_default_metadata_cache_size() -> usize {
...
mod tests {
    ...
    fn test_cache_size_defaults() {
        assert_eq!(rgw_lancedb_default_index_cache_size(), 256 * 1024 * 1024);
        assert_eq!(rgw_lancedb_default_metadata_cache_size(), 128 * 1024 * 1024);
    }

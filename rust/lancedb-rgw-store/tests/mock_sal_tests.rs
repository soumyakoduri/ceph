...
//! Comprehensive unit tests for lancedb-rgw-store using mock SAL
//!
...
use lancedb_rgw_store::{
    create_rgw_registry, create_rgw_session, create_rgw_session_with_cache,
    ffi::{OwnedRGWBuffer, RGWBuffer, RGWListResult, RGWObjectMeta},
    rgw_lancedb_create_registry, rgw_lancedb_default_index_cache_size,
    rgw_lancedb_default_metadata_cache_size, rgw_lancedb_registry_free, lancedb_rgw_store_version,
...
mod object_store_tests {
    ...
    #[test]
    fn test_store_creation_with_prefix() {
        let store = create_test_store_with_prefix("test-bucket", "my-prefix/");
        assert_eq!(
            format!("{}", store),
    ...
    #[test]
    fn test_store_prefix_accessor() {
        let store = create_test_store_with_prefix("bucket", "vectors/");
        assert_eq!(store.prefix(), "vectors/");
    }
    ...
    #[tokio::test]
    async fn test_put_opts_overwrite_mode() {
        let store = create_test_store("test-bucket");
        let location = Path::from("overwrite.txt");
        ...
        let result = store.put_opts(&location, payload, opts).await;
        assert!(result.is_ok());
    }

    #[tokio::test]
    async fn test_put_opts_create_mode() {
        let store = create_test_store("test-bucket");
        let location = Path::from("create-only.txt");
        let data = Bytes::from("data");
        let payload = PutPayload::from(data);
        let opts = PutOptions {
            mode: PutMode::Create,
        ...
        let result = store.put_opts(&location, payload, opts).await;
        // Mock returns success (no existing object to conflict)
        assert!(result.is_ok());
    }
    ...
    async fn test_list_with_prefix() {
        let store = create_test_store("test-bucket");
        let prefix = Path::from("data/vectors/");
    ...
    async fn test_copy_if_not_exists() {
        ...
        // Mock returns success for conditional copy
    ...
    async fn test_rename_if_not_exists() {
        ...
        let result = store.rename_if_not_exists(&from, &to).await;
        assert!(result.is_ok());
    }
    ...
    async fn test_path_with_unicode() {
        let store = create_test_store("test-bucket");
        let location = Path::from("data/fichier-test.txt");
    ...
    async fn test_path_with_special_chars() {
        let store = create_test_store("test-bucket");
        let location = Path::from("path/with spaces/file.txt");
...
mod multipart_tests {
    ...
    #[tokio::test]
    async fn test_init_multipart() {
        let store = create_test_store("test-bucket");
        let location = Path::from("large-file.bin");

        let result = store.put_multipart(&location).await;
        // Mock returns success for multipart init
        assert!(result.is_ok());
    }

    #[tokio::test]
    async fn test_multipart_put_part() {
        let store = create_test_store("test-bucket");
        let location = Path::from("large-file.bin");
        ...
        let payload = PutPayload::from(data);
        let part_future = upload.put_part(payload);

        let result = part_future.await;
        assert!(result.is_ok());
    }
...
mod c_api_tests {
    ...
    fn test_c_api_null_driver() {
        let registry =
            unsafe { rgw_lancedb_create_registry(std::ptr::null_mut(), std::ptr::null()) };
    ...
    fn test_c_api_registry_lifecycle() {
        ...
        let registry = unsafe { rgw_lancedb_create_registry(fake_driver, fake_dpp) };
        assert!(!registry.is_null());

        // Free should not crash
        unsafe { rgw_lancedb_registry_free(registry) };
    }

    #[test]
    fn test_c_api_default_cache_sizes() {
        let index_size = rgw_lancedb_default_index_cache_size();
        let metadata_size = rgw_lancedb_default_metadata_cache_size();
    ...
    fn test_c_api_free_null_registry() {
        ...
        unsafe { rgw_lancedb_registry_free(std::ptr::null_mut()) };
    }
}
...
mod error_tests {
    ...
    fn test_errno_mapping_coverage() {
        ...
        // ENOSYS -> NotSupported
        let err = store.errno_to_error_for_test(-38, &path, "operation");
...
mod ffi_tests {
    use super::*;
    use lancedb_rgw_store::ffi::RGWListEntry;
    ...
    #[test]
    fn test_rgw_buffer_size() {
        // RGWBuffer should be 3 pointers/size_t in size
        assert_eq!(
            std::mem::size_of::<RGWBuffer>(),
            std::mem::size_of::<*mut u8>() + 2 * std::mem::size_of::<usize>()
        );
    }

    #[test]
    fn test_rgw_list_entry_size() {
        // RGWListEntry: char* + uint64_t + int64_t
        let expected = std::mem::size_of::<*mut std::os::raw::c_char>()
            + std::mem::size_of::<u64>()
            + std::mem::size_of::<i64>();
        assert_eq!(std::mem::size_of::<RGWListEntry>(), expected);
    }
}

...
def test_sync_bucket_select_zones():
    """
    test_sync_bucket_select_zones:
    ...
    create_objects(zcA, bucketA, objnamesA, content)
    create_objects(zcB, bucketA, objnamesB, content)

    zonegroup_meta_checkpoint(zonegroup)
    zone_data_checkpoint(zoneB, zoneA)

    # verify that objnamesA are synced to bucketA in zoneB
    bucket = get_bucket(zcB, bucketA.name)
    check_objects_exist(bucket, objnamesA, content)

    # verify that objnamesB are not synced to bucketA in zoneA
...
def test_sync_bucket_from_different_bucket():
    """
    test_sync_bucket_from_different_bucket:
        similar to 'test_sync_pipe_different_buckets' but at bucket level

        symmetrical flow but do not enable it at zonegroup level
        configure another policy at bucketA level with pipe set from
    ...
    create_objects(zcB, bucketA, objnamesA, content)
    create_objects(zcB, bucketB, objnamesB, content)

...
public:
  ...
  static int exists(CephContext* cct, RGWSI_Cls& cls) {
    auto num_shards = cct->_conf->rgw_data_log_num_shards;
...
class RGWDataChangesFIFO final : public RGWDataChangesBE {
  ...
  std::vector<std::unique_ptr<R::cls::fifo::FIFO>> fifos;
public:
  RGWDataChangesFIFO(CephContext* cct, R::RADOS& r,
		     const rgw_pool& log_pool)
    : RGWDataChangesBE(cct) {
    const auto shards = cct->_conf->rgw_data_log_num_shards;
    auto ioc = rgw_rados_acquire_pool(r, log_pool, false, null_yield,
				      true);
    if (!ioc)
      throw ioc.error();

    bs::error_code ec;
    fifos.reserve(shards);
    for (auto i = 0; i < shards; ++i) {
      auto oid = get_oid(i);
      fifos.emplace_back(R::cls::fifo::FIFO::create(r, *ioc, oid,
						    ca::use_blocked[ec]));
      if (ec) {
	lderr(cct) << __PRETTY_FUNCTION__
		   << ": failed to create fifo " << *ioc << "/" << oid
		   << ": " << ec.message() << dendl;
  ...
  static int exists(CephContext* cct, R::RADOS& r, const rgw_pool& log_pool) {
    auto num_shards = cct->_conf->rgw_data_log_num_shards;
    auto ioc = rgw_rados_acquire_pool(r, log_pool, false, null_yield,
				      false);
    if (!ioc) {
      if (ioc.error() == bs::errc::no_such_file_or_directory) {
	return 0;
      } else {
	lderr(cct) << __PRETTY_FUNCTION__
		   << ": failed to look up pool " << log_pool.name
		   << ioc.error().message() << dendl;
    ...
    for (auto i = 0; i < num_shards; ++i) {
      auto oid = get_oid(cct, i);
      R::cls::fifo::FIFO::open(r, *ioc, oid,
			       ca::use_blocked[ec]);
      if (!ec) {
	return 1;
      } else if (ec == bs::errc::no_such_file_or_directory ||
		 ec == bs::errc::no_message_available) {
	continue;
      } else {
	lderr(cct) << __PRETTY_FUNCTION__
		   << ": failed to open fifo " << *ioc << "/" << oid
		   << ": " << ec.message() << dendl;
  ...
  int push(int index, entries&& items) override {
    bs::error_code ec;
    fifos[index]->push(std::get<centries>(items), ca::use_blocked[ec]);
    if (ec) {
      lderr(cct) << __PRETTY_FUNCTION__
		 << ": failed to push to fifo " << get_oid(index)
		 << ": " << ec.message() << dendl;
    }
    return ceph::from_error_code(ec);
  }
  int push(int index, ceph::real_time,
	   ...
	   ceph::buffer::list&& bl) override {
    bs::error_code ec;
    fifos[index]->push(std::move(bl), ca::use_blocked[ec]);
    if (ec) {
      lderr(cct) << __PRETTY_FUNCTION__
		 << ": failed to push to fifo " << get_oid(index)
		 << ": " << ec.message() << dendl;
  ...
  int list(int index, int max_entries,
	   ...
	   std::string* out_marker, bool* truncated) override {
    bs::error_code ec;
    auto [log_entries, more] = fifos[index]->list(max_entries, marker,
						  ca::use_blocked[ec]);
    if (ec) {
      lderr(cct) << __PRETTY_FUNCTION__
		 << ": failed to list fifo " << get_oid(index)
		 << ": " << ec.message() << dendl;
      return ceph::from_error_code(ec);
    }
  ...
  int get_info(int index, RGWDataChangesLogInfo *info) override {
    ...
    bs::error_code ec;
    fifo->read_meta(ca::use_blocked[ec]);
    if (ec) {
      lderr(cct) << __PRETTY_FUNCTION__
		 << ": failed to read FIFO metadata " << get_oid(index)
		 << ": " << ec.message() << dendl;
      return ceph::from_error_code(ec);
    }
    auto m = fifo->meta();
    ...
    if (p < 0) {
      info->marker = neorados::cls::fifo::marker{}.to_string();
      info->last_update = ceph::real_clock::zero();
      return 0;
    }
    auto h = fifo->get_part_info(p, ca::use_blocked[ec]);
    if (ec) {
      lderr(cct) << __PRETTY_FUNCTION__
		 << ": failed get part info: " << get_oid(index) << "/" << p
		 << ": " << ec.message() << dendl;
    } else {
      info->marker = neorados::cls::fifo::marker{p, h.max_ofs}.to_string();
  ...
  int trim(int index, std::string_view marker) override {
    bs::error_code ec;
    fifos[index]->trim(marker, ca::use_blocked[ec]);
    if (ec) {
      lderr(cct) << __PRETTY_FUNCTION__
		 << ": failed get part info: " << get_oid(index)
		 << ": " << ec.message() << dendl;
    }
    return ceph::from_error_code(ec);
  }
  int trim(int index, std::string_view marker,
	   librados::AioCompletion* c) override {
    if (marker == neorados::cls::fifo::marker(0, 0).to_string()) {
      boost::asio::post(c);
    } else {
      fifos[index]->trim(marker, c);
    }
    return 0;
  }
  std::string_view max_marker() const override {
    static const std::string mm =
      neorados::cls::fifo::marker::max().to_string();
    return std::string_view(mm);
  }
};

class RGWDataChangesLegacyFIFO final : public RGWDataChangesBE {
  using centries = std::vector<ceph::buffer::list>;
  std::vector<std::unique_ptr<rgw::cls::fifo::FIFO>> fifos;
public:
  RGWDataChangesLegacyFIFO(CephContext* cct, librados::Rados* rados,
			   const rgw_pool& log_pool)
    : RGWDataChangesBE(cct) {
  ...
  ~RGWDataChangesLegacyFIFO() override = default;
...
int RGWDataChangesLog::start(const RGWZone* _zone,
			     ...
			     RGWSI_Cls *cls, R::RADOS* rados,
			     librados::Rados* lr)
{
  zone = _zone;
  assert(zone);
  auto backing = cct->_conf.get_val<std::string>("rgw_datalog_backing");
  ...
  auto r = RGWDataChangesOmap::exists(cct, *cls);
  if (r < 0) {
    lderr(cct) << __PRETTY_FUNCTION__
  ...
  bool omapexists = (r > 0);
  r = RGWDataChangesFIFO::exists(cct, *rados, log_pool);
  if (r < 0) {
    lderr(cct) << __PRETTY_FUNCTION__
  ...
  if ((backing == "fifo" || backing == "legacyfifo") && omapexists) {
    lderr(cct) << __PRETTY_FUNCTION__
  ...
  try {
    ...
    } else if (backing != "legacyfifo") {
      be = std::make_unique<RGWDataChangesFIFO>(cct, *rados, log_pool);
    } else if (backing == "legacyfifo") {
      be = std::make_unique<RGWDataChangesLegacyFIFO>(cct, lr, log_pool);
    }

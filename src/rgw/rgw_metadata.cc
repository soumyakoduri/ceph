...
public:
  ...
  int push(int index, entries& items, librados::AioCompletion *&completion) override {
    ...
    if (r < 0) {
      lderr(cct) << __PRETTY_FUNCTION__
		 << ": AIO completion error for push of " << get_shard_oid(index)
		 << ": " << cpp_strerror(-r) << dendl;
    }
...
/*      log_entry.id = entry.marker;
  ...
  int get_info_async(int index, RGWMetadataLogInfoCompletion *completion) override {
    ...
    if (p < 0) {
      header.max_marker = rgw::cls::fifo::marker{}.to_string();
      header.max_time = utime_t(ceph::real_clock::zero());
      return 0;
    }
    librados::AioCompletion* c = completion->get_completion();
    rgw::cls::fifo::part_info h;
    r = fifo->get_part_info(p, &h, c);
    if (r < 0) {
      lderr(cct) << __PRETTY_FUNCTION__
		 << ": unable to get part info: " << get_shard_oid(index) << "/" << p
		 << ": " << cpp_strerror(-r) << dendl;

...
int RGWZoneGroupPlacementTier::update_params(const JSONFormattable& config)
{
  ...
  if (config.exists("cloud_restore_object_hold_period")) {
    r = conf_to_uint64(config, "cloud_restore_object_hold_period", &cloud_restore_object_hold_period);
    if (r < 0) {
      cloud_restore_object_hold_period = DEFAULT_RESTORE_OBJECT_HOLD_PERIOD;
    }
  }

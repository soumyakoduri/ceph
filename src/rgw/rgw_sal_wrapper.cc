...
// Helper to get optional_yield from opaque yield_ctx pointer.
// NULL → null_yield (blocks the calling thread, correct for Rust/Tokio's spawn_blocking).
// Non-NULL → dereference as optional_yield* (correct for RGW Beast/ASIO coroutine callers).
...
// Helper to generate a unique tag for write operations
static std::string generate_unique_tag() {
  uuid_d uuid;
  uuid.generate_random();
  char buf[37];  // UUID string is 36 chars + null terminator
  uuid.print(buf);
...
int rgw_put_object( void* driver_ptr, const void* dpp_ptr,
      void* yield_ctx, const char* bucket_name, const char* key,
  ...
  try {
    ...
    // Generate unique tag for write idempotency
    std::string unique_tag = generate_unique_tag();
...
int rgw_copy_object( void* driver_ptr, const void* dpp_ptr,
      void* yield_ctx, const char* src_bucket_name, const char* src_key,
...
int rgw_copy_object_conditional( void* driver_ptr, const void* dpp_ptr,
      void* yield_ctx, const char* src_bucket_name, const char* src_key,
      const char* dst_bucket_name, const char* dst_key, const char* if_match,

...
class RGWLCStreamGetCloud
{
  ...
  public:
  RGWLCStreamGetCloud(const DoutPrefixProvider *_dpp,
      bool _head,
      const rgw_lc_obj_properties&  _obj_properties,
      std::shared_ptr<RGWRESTConn> _conn,
      rgw::sal::Object* _dest_obj) :
    dpp(_dpp), head(_head), obj_properties(_obj_properties), conn(_conn), dest_obj(_dest_obj) {}
  int init(const DoutPrefixProvider *dpp);
  int is_already_tiered();
};

/* Read object locally & also initialize dest rest obj */
...
class RGWLCStreamPutCloud
{
  ...
  public:
  RGWLCStreamPutCloud(const DoutPrefixProvider *_dpp,
      const rgw_lc_obj_properties&  _obj_properties,
      std::shared_ptr<RGWRESTConn> _conn,
      rgw::sal::Object* _dest_obj) :
  ...
  /* Read and write to Cloud endpoint */
  class RGWLCStreamRWCloud {
    const DoutPrefixProvider *dpp;
    string url;
    std::shared_ptr<RGWLCStreamRead> readf;
    std::shared_ptr<RGWLCStreamPutCloud> writef;
    bufferlist bl;
    ...
    public:
    RGWLCStreamRWCloud(const DoutPrefixProvider *_dpp,
        std::shared_ptr<RGWLCStreamRead>& _readf,
        shared_ptr<RGWLCStreamPutCloud>& _writef) : dpp(_dpp),
  ...
  class RGWLCStreamObjToCloudPlain {
    RGWLCCloudTierCtx& tier_ctx;

    std::shared_ptr<RGWLCStreamRead> readf;
    std::shared_ptr<RGWLCStreamPutCloud> writef;
    std::unique_ptr<RGWLCStreamRWCloud> rwf;
    ...
    public:
    RGWLCStreamObjToCloudPlain(RGWLCCloudTierCtx& _tier_ctx)
      : tier_ctx(_tier_ctx),
  ...
  class RGWLCStreamObjToCloudMultipartPart {
    RGWLCCloudTierCtx& tier_ctx;
    ...
    public:
    RGWLCStreamObjToCloudMultipartPart(RGWLCCloudTierCtx& _tier_ctx, const string& _upload_id,
  ...
  class RGWLCAbortMultipart {
    ...
    public:
    RGWLCAbortMultipart(const DoutPrefixProvider *_dpp,
  ...
  class RGWLCInitMultipart {
    ...
    public:
    RGWLCInitMultipart(const DoutPrefixProvider *_dpp,
        RGWRESTConn *_dest_conn, const rgw_obj& _dest_obj,
  ...
  class RGWLCCompleteMultipart {
    ...
    public:
    RGWLCCompleteMultipart(const DoutPrefixProvider *_dpp,
        RGWRESTConn *_dest_conn, const rgw_obj& _dest_obj,
  ...
  class RGWLCStreamAbortMultipartUpload {
    RGWLCCloudTierCtx& tier_ctx;
    const rgw_obj dest_obj;
    const rgw_raw_obj status_obj;

    string upload_id;
    std::unique_ptr<RGWLCAbortMultipart> abort_mp;

    int retcode;

    public:

    RGWLCStreamAbortMultipartUpload(RGWLCCloudTierCtx& _tier_ctx,
  ...
  class RGWLCStreamObjToCloudMultipart {
    RGWLCCloudTierCtx& tier_ctx;
    ...
    public:
    RGWLCStreamObjToCloudMultipart(RGWLCCloudTierCtx& _tier_ctx)
      : tier_ctx(_tier_ctx),
      obj_properties(tier_ctx.o.meta.mtime, tier_ctx.o.meta.etag,
          tier_ctx.o.versioned_epoch, tier_ctx.acl_mappings,
          tier_ctx.target_storage_class){}
...
class RGWLCCheckCloud {
  ...
  std::unique_ptr<rgw::sal::Bucket> dest_bucket;
  std::unique_ptr<rgw::sal::Object> dest_obj;
  std::unique_ptr<RGWLCStreamGetCloud> getf;

  public:
  RGWLCCheckCloud(RGWLCCloudTierCtx& _tier_ctx, bool *_al_ti) :
  ...
  class RGWLCCloudTier {
    ...
    bufferlist out_bl;
    int retcode;
    std::unique_ptr<RGWLCCheckCloud> chk_cloud;
    std::unique_ptr<RGWLCStreamObjToCloudPlain> cloud_tier_plain;
    std::unique_ptr<RGWLCStreamObjToCloudMultipart> cloud_tier_mp;

...
/**
 * @file rgw_draft_2__sal_wrapper.cc
 * @brief C FFI wrapper implementation for RGW SAL with Quota Enforcement
 *
 ...
 * - Enforcing quota checks before write operations
 ...
 * - Quota enforcement follows RGWOp::init_quota() pattern
...
/**
 * Initialize quota structure for a bucket operation
 *
 * Follows the pattern from RGWOp::init_quota() in rgw_op.cc
 * This function merges driver quotas, user quotas, and bucket-specific quotas
 * to create a unified quota policy for the operation.
 *
 * @param dpp Logging provider
 * @param driver SAL driver
 * @param bucket Bucket for the operation
 * @param quota Output quota structure
 * @param y Optional yield context
 * @return 0 on success, negative errno on failure
 */
static int init_quota_for_bucket(const DoutPrefixProvider* dpp,
                                  Driver* driver,
                                  Bucket* bucket,
                                  RGWQuota& quota,
                                  optional_yield y)
{
  // Get bucket owner's quota information
  RGWQuota user_quotas;
  int ret = get_owner_quota_info(dpp, y, driver,
                                  bucket->get_info().owner,
                                  user_quotas);
  if (ret < 0) {
    ldpp_dout(dpp, 0) << "ERROR: failed to get owner quota info: " << ret << dendl;
    return ret;
  }

  // Get driver's default quota configuration
  driver->get_quota(quota);

  // Apply bucket-specific quota if enabled, otherwise use user's bucket quota
  if (bucket->get_info().quota.enabled) {
    quota.bucket_quota = bucket->get_info().quota;
    ldpp_dout(dpp, 20) << "Using bucket-specific quota" << dendl;
  } else if (user_quotas.bucket_quota.enabled) {
    quota.bucket_quota = user_quotas.bucket_quota;
    ldpp_dout(dpp, 20) << "Using user bucket quota" << dendl;
  }
...
int rgw_put_object(void* driver_ptr, const void* dpp_ptr, const char* bucket_name,
  ...
  try {
    ...
    // Initialize and check quota before creating processor
    RGWQuota quota;
    ret = init_quota_for_bucket(dpp, driver, bucket.get(), quota, null_yield);
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "ERROR: failed to initialize quota: " << ret << dendl;
      return ret;
    }

    // Check quota enforcement
    ret = bucket->check_quota(dpp, quota, data_len, null_yield);
    if (ret < 0) {
      ldpp_dout(dpp, 0) << "ERROR: quota check failed (object size: "
                        << data_len << " bytes): " << ret << dendl;
      return ret;  // Returns -EDQUOT if quota exceeded
    }
    ...
    // Quota check passed, object written successfully
    ldpp_dout(dpp, 20) << "Object written successfully, size: " << data_len
                       << " bytes, etag: " << etag_str << dendl;
...
int rgw_delete_object(void* driver_ptr, const void* dpp_ptr, const char* bucket_name,
  ...
  try {
    ...
    // Delete the object
    // Note: No quota check needed for delete operations (they free space)
    ...
    ldpp_dout(dpp, 20) << "Object deleted successfully: " << key << dendl;
    return 0;
